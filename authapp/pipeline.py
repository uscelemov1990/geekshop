...
def save_user_profile(backend, user, respinse, *args, **kwargs):
    if backend.name != 'vk-oauth2':
        return

    utl_method = 'http://api.vk.com/method/'
    fields = ','.join(['bdate', 'sex', 'about'])
    access_token = response.get('access_token')

    api_url = f'{utl_method}users.get?fields={fields}&access_token={access_token}&v=5.131'
